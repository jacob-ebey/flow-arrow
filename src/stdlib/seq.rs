...
pub const INNER_LENGTH: StdSymbol = node(MODULE, "inner_length", "Seq[Seq[V]]", "i64");
...
pub const TAKE: StdSymbol = node(MODULE, "take", "(Seq[V],i64)", "Seq[V]");
pub const DROP: StdSymbol = node(MODULE, "drop", "(Seq[V],i64)", "Seq[V]");
pub const FILL: StdSymbol = node(MODULE, "fill", "(V,i64)", "Seq[V]");
pub const SLICE: StdSymbol = node(MODULE, "slice", "(Seq[V],i64,i64)", "Seq[V]");
pub const LAST: StdSymbol = node(MODULE, "last", "Seq[V]", "Faultable[V]");
pub const GET: StdSymbol = node(MODULE, "get", "(Seq[V],i64)", "V");
pub const GET_OR: StdSymbol = node(MODULE, "get_or", "(Seq[V],i64,V)", "V");
pub const AT: StdSymbol = node(MODULE, "at", "(Seq[V],i64)", "Faultable[V]");
pub const APPEND: StdSymbol = node(MODULE, "append", "(Seq[V],V)", "Seq[V]");
pub const SET: StdSymbol = node(MODULE, "set", "(Seq[V],i64,V)", "Seq[V]");
