...
static FaFault fa_stream_fault(FaBytes path, const char *operation) {
...
static FaFaultable_Stream fa_open_file(FaBytes path) {
  if (memchr(path.bytes, '\0', path.len)) {
    return FaFaultable_Stream_fault(fa_fault_cstr("open_file: path contains NUL byte"));
  }
  char *path_c = fa_copy_bytes(path.bytes, path.len);
  FILE *file = fopen(path_c, "rb");
  free(path_c);
  if (!file) return FaFaultable_Stream_fault(fa_stream_fault(path, "open_file"));

  FaStream stream;
  stream.file = file;
  stream.fd = fileno(file);
  stream.path = path;
  return FaFaultable_Stream_ok(stream);
}
...
static FaFaultable_Int fa_stream_size(FaStream stream) {
  ...
  struct stat st;
  if (fstat(stream.fd, &st) != 0) return FaFaultable_Int_fault(fa_stream_fault(stream.path, "size"));
...
static FaFaultable_Bytes fa_stream_read_at(FaStream stream, int64_t offset, int64_t len) {
  ...
  while (done < (size_t)len) {
    ...
    if (read < 0) {
      FaFault fault = fa_stream_fault(stream.path, "read_at");
      free(buffer);
...
static FaFaultable_Int fa_copy_stream_to_file(FaStream stream, FaBytes output_path) {
  ...
  free(path_c);
  if (!output) return FaFaultable_Int_fault(fa_stream_fault(output_path, "copy_to_file"));
  ...
  for (;;) {
    ...
    if (read > 0) {
      ...
      if (written != read) {
        FaFault fault = fa_stream_fault(output_path, "copy_to_file");
        free(buffer);
        fclose(output);
    ...
    if (read < FA_STREAM_BUFFER_SIZE) {
      if (ferror(stream.file)) {
        FaFault fault = fa_stream_fault(stream.path, "copy_to_file");
        free(buffer);
        fclose(output);
        return FaFaultable_Int_fault(fault);
      }
  ...
  free(buffer);
  if (fclose(output) != 0) {
    return FaFaultable_Int_fault(fa_stream_fault(output_path, "copy_to_file"));
  }
...
static FaFaultable_Int fa_close_stream(FaStream stream) {
  ...
  if (fclose(stream.file) != 0) {
    return FaFaultable_Int_fault(fa_stream_fault(stream.path, "close"));
  }
  return FaFaultable_Int_ok(0);
}
